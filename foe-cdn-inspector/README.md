...
GB Update Tracker is generated by this project. From `foe-cdn-inspector/`, run the refresh command to check for updates and rebuild the dashboard when a new report is available:

```bash
python3 -B -u -m foe_cdn_inspector refresh
```
...
### Validate and publish a refresh

A successful command returns `status: refreshed` or `status: up_to_date`. Check the reported newest report, history range, description count, and icon count when present. `up_to_date` still updates the successful-check date; it does not mean a new report was published.

Verify that `snapshots/latest.txt` names the expected report, both `dashboard/index.html` and `dashboard/history.html` match that snapshot, and `string_results.failures` and `history_results.failures` in its `dashboard-state.json` are empty. Review `git diff --check` and the dashboard diff before publishing. When refresh code changes, run `python3 -B -m unittest discover -s tests -v`.

When a commit and push are requested, include both generated dashboard HTML files and any related source, tests, or documentation changed for the refresh. Stage explicit paths; `.cache/`, `results/`, and `snapshots/` are local working data and remain ignored. Keep unrelated edits and preview assets out of the commit. A local refresh alone does not publish the site.

### Beta bootstrap compatibility

The public beta landing page can embed `ONELPS_RUNTIME_CONFIG` as either plain JSON or Base64-encoded UTF-8 JSON inside `JSON.parse(new TextDecoder().decode(Uint8Array.from(atob(...), c => c.charCodeAt(0))))`. The parser supports both formats without executing remote JavaScript and still requires market `zz` before collecting a new report.

If bootstrap verification fails, inspect the current public page and correct the parser with a regression test before rerunning `refresh`. Keep the market check enabled; neither `scan` nor manually changing the dashboard date replaces a successful refresh. `--force` rebuilds an unchanged report and does not bypass bootstrap verification.
