...
const createPageLogs = offset => {
  ...
  return Array.from({ length: 10 }, (_unused, index) => {
    // Two legitimate, ID-less records can have the same visible signature.
    // Page stitching must preserve both because they are not a proven boundary overlap.
    const visibleIndex = page === 0 && index < 2 ? 0 : index;
    return {
      player: { player_id: 855340115, name: 'zpwd' },
      resource: 'fusion_reactors',
      amount: 100 + offset + visibleIndex,
      action: 'Guild treasury donation',
      createdAt: new Date(first.getTime() - visibleIndex * 60_000).toISOString(),
    };
  });
};
